...
export default function QAPaint() {
  ...
  const fetchReports = () => {
    fetch('http://localhost:5000/api/qa/paint')
      .then(res => res.json())
  ...
  const handleSubmit = (e) => {
    ...
    if (file) formData.append('qa_paint', file);

    const url = editMode.isEditing 
      ? `http://localhost:5000/api/qa/paint/${editMode.id}` 
      : 'http://localhost:5000/api/qa/paint';
  ...
  const handleDelete = (id) => {
    if(window.confirm("Are you sure you want to delete this report? This cannot be undone.")) {
      fetch(`http://localhost:5000/api/qa/paint/${id}`, { method: 'DELETE' })
        .then(() => {
  ...
  const handleViewPdf = async (filename, docTitle) => {
    ...
    try {
      ...
      const targetUrl = `http://localhost:5000/api/qa/paint/view-pdf/${safeFilename}`;
      ...
      setMsg({ type: 'error', text: 'Failed to load PDF. Please check server connection.' });
  ...
  return (
    <div className="space-y-6">
      ...
      <div className={`bg-white rounded-2xl shadow-sm border p-8 transition-all ${editMode.isEditing ? 'border-amber-300 ring-4 ring-amber-50' : 'border-slate-200'}`}>
        <div className="flex justify-between items-center mb-6">
          <div>
            <h2 className="text-xl font-bold text-slate-800 flex items-center gap-2">
              <DocumentIcon /> {editMode.isEditing ? 'Modify Paint Shop Audit' : 'Upload Paint Shop Audit'}
            </h2>
            <p className="text-sm text-slate-500 mt-1">Manage official QA audit documentation for Paint Shop parameters and logs.</p>
          </div>
          {editMode.isEditing && (
        ...
        <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-12 gap-4 items-end">
          <div className="md:col-span-5">
            ...
            <input type="text" value={title} onChange={(e) => setTitle(e.target.value)} required placeholder="e.g., Q4 Paint Gloss & Adhesion Audit" className={inputClass} />
          </div>
          ...
          <div className="md:col-span-3">
            <button type="submit" disabled={isLoading} className={`w-full text-white font-bold py-3 px-4 rounded-xl shadow-sm transition-all flex items-center justify-center gap-2 ${editMode.isEditing ? 'bg-amber-600 hover:bg-amber-700' : 'bg-indigo-600 hover:bg-indigo-700'}`}>
      ...
      {/* Library Layout Wrapper with Search bar */}
      <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
        ...
        <div className="px-8 py-5 border-b border-slate-100 bg-slate-50 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div>
            <h3 className="text-lg font-bold text-slate-800">Paint Shop QA Document Library</h3>
            ...
            {filteredReports.map((doc) => (
              <tr key={doc.id} className="hover:bg-slate-50 transition-colors">
                <td className="py-4 px-8 font-medium text-slate-800 flex items-center gap-3">
                  <div className="w-8 h-8 rounded bg-amber-50 text-amber-600 flex items-center justify-center shrink-0 text-xs font-bold border border-amber-100">PDF</div>
                  {doc.title}
                </td>
                ...
                <td className="py-4 px-8 flex justify-center gap-3">
                  ...
                  <button onClick={() => handleEditClick(doc)} className="p-2 text-slate-400 hover:text-amber-600 hover:bg-amber-50 rounded-lg transition-all" title="Edit Record">
                    <EditIcon />
                  </button>
