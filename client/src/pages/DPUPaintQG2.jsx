// client/src/pages/DPUPaintQG2.jsx
...
export default function DPUPaintQG2() {
  ...
  // Form State
  const [formData, setFormData] = useState({ id: null, paintQg2No: '', date: '', totalVehicles: '', cumulative: '', ratio: '' });
  ...
  const fetchData = useCallback(() => {
    apiFetch('http://localhost:5000/api/dpu/paint-qg2')
      .then(res => res.json())
  ...
  const handleOpenForm = (record = null) => {
    if (record) {
      setFormData({ 
        id: record.id, paintQg2No: record.paintQg2No, date: record.date, 
        totalVehicles: record.totalVehicles, cumulative: record.cumulative, ratio: record.ratio 
      });
    } else {
      setFormData({ 
        id: null, paintQg2No: `SEWL-QG2-${Date.now().toString().slice(-4)}`, 
  ...
  const handleSubmit = (e) => {
    ...
    payload.append('paintQg2No', formData.paintQg2No);
    ...
    const url = formData.id ? `http://localhost:5000/api/dpu/paint-qg2/${formData.id}` : 'http://localhost:5000/api/dpu/paint-qg2';
  ...
  const handleDelete = (id) => {
    if (window.confirm('Are you sure you want to delete this record? This action cannot be undone.')) {
      apiFetch(`http://localhost:5000/api/dpu/paint-qg2/${id}`, { method: 'DELETE' })
  ...
  const handleViewPdf = async (filename, docTitle) => {
    ...
    try {
      ...
      const targetUrl = `http://localhost:5000/api/dpu/paint-qg2/view-pdf/${safeFilename}`;
  ...
  const filteredAndSortedData = records
    .filter(item => {
      ...
      if (searchTerm) {
        ...
        searchMatch = (item.paintQg2No && item.paintQg2No.toLowerCase().includes(lowerSearch));
      }
        ...
        <div className="animate-fade-in">
          ...
          <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden flex flex-col">
            ...
            <div className="p-6 border-b border-slate-100 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-slate-50/50">
              <div>
                <h2 className="text-lg font-bold text-slate-800">Paint QG2 Line DPU Records</h2>
                <p className="text-sm text-slate-500">Track and analyze Defects Per Unit across the Paint QG2 process.</p>
              </div>
            ...
            <div className="p-6 border-b border-slate-100 flex flex-col lg:flex-row justify-between items-end gap-6">
              ...
              <div className="flex flex-col sm:flex-row gap-4 items-center w-full lg:w-auto">
                ...
                <div className="relative w-full sm:w-64">
                  ...
                  <input type="text" placeholder="Search Paint QG2 No..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full pl-9 pr-4 py-2.5 border border-slate-200 bg-slate-50 rounded-lg text-sm focus:ring-2 focus:ring-indigo-500 outline-none" />
                </div>
              </div>
            </div>
            ...
            <div className="overflow-x-auto min-h-[300px]">
              <table className="w-full text-left text-sm text-slate-600 min-w-max">
                <thead className="border-b border-slate-200 text-xs font-bold text-slate-500 uppercase bg-white sticky top-0 shadow-sm z-10">
                  <tr>
                    <th className="py-4 px-6 cursor-pointer hover:text-indigo-600 transition-colors" onClick={() => toggleSort('paintQg2No')}>Paint QG2# <SortArrow active={sortField==='paintQg2No'} direction={sortDirection} /></th>
                ...
                <tbody className="divide-y divide-slate-100">
                  {currentItems.map(row => (
                    <tr key={row.id} className="hover:bg-slate-50/80 transition-colors group">
                      <td className="py-4 px-6 font-bold text-slate-800">{row.paintQg2No}</td>
                      ...
                      <td className="py-4 px-6 text-center">
                        {row.attachmentFilename ? (
                          <button onClick={() => handleViewPdf(row.attachmentFilename, row.paintQg2No)} className="text-indigo-500 hover:text-white hover:bg-indigo-500 p-2 rounded-lg transition-all shadow-sm border border-indigo-100 hover:border-indigo-500" title="View Document">
                            <EyeIcon />
                          </button>
                        ) : (
        ...
        <div className="max-w-4xl mx-auto animate-fade-in">
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
            <div className="p-6 border-b border-slate-100 bg-slate-50 flex justify-between items-center">
              <div>
                <h2 className="text-xl font-bold text-slate-800">{formData.id ? 'Edit Paint QG2 DPU Record' : 'New Paint QG2 DPU Record'}</h2>
            ...
            <form onSubmit={handleSubmit} className="p-8 space-y-8">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">Paint QG2 No (Auto-Generated)</label>
                  <input type="text" value={formData.paintQg2No} readOnly className="w-full border border-slate-200 bg-slate-100 rounded-xl p-3.5 text-sm font-mono font-bold text-slate-500 outline-none cursor-not-allowed" />
                </div>
                <div>
