...
export default function QAAssembly() {
  ...
  const fetchReports = () => {
    fetch('http://localhost:5000/api/qa/assembly')
      .then(res => res.json())
  ...
  const handleSubmit = (e) => {
    ...
    if (file) formData.append('qa_assembly', file);

    const url = editMode.isEditing 
      ? `http://localhost:5000/api/qa/assembly/${editMode.id}` 
      : 'http://localhost:5000/api/qa/assembly';
  ...
  const handleDelete = (id) => {
    if(window.confirm("Are you sure you want to delete this report? This cannot be undone.")) {
      fetch(`http://localhost:5000/api/qa/assembly/${id}`, { method: 'DELETE' })
        .then(() => {
  ...
  const handleViewPdf = async (filename, docTitle) => {
    ...
    try {
      ...
      const targetUrl = `http://localhost:5000/api/qa/assembly/view-pdf/${safeFilename}`;
  ...
  // --- NEW: Filter Reports via Search Input ---
    ...
    <div className="space-y-6">
      ...
      <div className={`bg-white rounded-2xl shadow-sm border p-8 transition-all ${editMode.isEditing ? 'border-emerald-300 ring-4 ring-emerald-50' : 'border-slate-200'}`}>
        <div className="flex justify-between items-center mb-6">
          <div>
            <h2 className="text-xl font-bold text-slate-800 flex items-center gap-2">
              <DocumentIcon /> {editMode.isEditing ? 'Modify Assembly Audit' : 'Upload Assembly Audit'}
            </h2>
            <p className="text-sm text-slate-500 mt-1">Manage official QA audit documentation for the Assembly Line configurations.</p>
          </div>
          {editMode.isEditing && (
        ...
        <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-12 gap-4 items-end">
          <div className="md:col-span-5">
            ...
            <input type="text" value={title} onChange={(e) => setTitle(e.target.value)} required placeholder="e.g., Nov 2026 Assembly Line Audit" className={inputClass} />
          </div>
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
        ...
        <div className="px-8 py-5 border-b border-slate-100 bg-slate-50 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div>
            <h3 className="text-lg font-bold text-slate-800">Assembly QA Document Library</h3>
            ...
            {filteredReports.map((doc) => (
              <tr key={doc.id} className="hover:bg-slate-50 transition-colors">
                <td className="py-4 px-8 font-medium text-slate-800 flex items-center gap-3">
                  <div className="w-8 h-8 rounded bg-indigo-50 text-indigo-600 flex items-center justify-center shrink-0 text-xs font-bold border border-indigo-100">PDF</div>
                  {doc.title}
                </td>
