...
const SortArrow = ({ active, direction }) => (
  <span className={`inline-block ml-1 text-xs ${active ? 'text-fuchsia-500' : 'text-slate-400'}`}>
    {direction === 'asc' ? '↑' : direction === 'desc' ? '↓' : '↕'}
  </span>
);

export default function BigDataLibrary() {
  ...
  const [isFetching, setIsFetching] = useState(true); // initial load spinner
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    if (!title.trim()) return showToast('Please provide a dataset title.', 'error');
    if (!editMode.isEditing && !file) return showToast('Please select a PDF file.', 'error');

    setIsLoading(true);
    const formData = new FormData();
    formData.append('title', title.trim());
    if (file) formData.append('bigdata_file', file);

    const url = editMode.isEditing 
      ? `http://localhost:5000/api/bigdata/${editMode.id}` 
      : 'http://localhost:5000/api/bigdata';
    ...
    apiFetch(url, { method, body: formData })
      ...
      .then(data => {
        ...
        showToast(editMode.isEditing ? 'Dataset updated successfully!' : 'Dataset uploaded successfully!');
        handleCancelEdit();
        fetchReports();
      })
      .catch(() => {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const res = await apiFetch(`http://localhost:5000/api/bigdata/${id}`, { method: 'DELETE' });
  ...
  const handleViewPdf = async (filename, docTitle) => {
    ...
    try {
      ...
      const response = await apiFetch(`http://localhost:5000/api/bigdata/view-pdf/${safeFilename}`);
  ...
  const filteredAndSortedData = reports
    .filter(doc => {
      ...
      return doc.title.toLowerCase().includes(searchLower) || doc.uploadDate.toLowerCase().includes(searchLower);
    })
    .sort((a, b) => {
  ...
  const inputClass = "w-full border border-slate-200 bg-slate-50 focus:bg-white rounded-xl p-3 text-sm text-slate-700 focus:ring-2 focus:ring-fuchsia-500 focus:border-fuchsia-500 outline-none transition-all";
  ...
  return (
    <div className="space-y-6 relative">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="bg-white rounded-2xl border border-slate-200 p-5 shadow-sm flex items-center gap-4">
          <div className="w-12 h-12 rounded-xl bg-fuchsia-50 flex items-center justify-center shrink-0">
            <DatabaseIcon className="text-fuchsia-600 w-6 h-6" />
          </div>
          <div>
            <p className="text-sm font-semibold text-slate-500">Total Datasets</p>
        ...
        <div className="bg-white rounded-2xl border border-slate-200 p-5 shadow-sm flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-2xl font-bold text-indigo-600">{recentlyAdded}</p>
          </div>
        </div>
      </div>
      ...
      <div className={`bg-white rounded-2xl shadow-sm border p-8 transition-all ${
        editMode.isEditing ? 'border-fuchsia-300 ring-4 ring-fuchsia-50' : 'border-slate-200'
      }`}>
        <div className="flex justify-between items-center mb-6">
          <div>
            <h2 className="text-xl font-bold text-slate-800 flex items-center gap-2">
              <UploadIcon /> {editMode.isEditing ? 'Update Big Data Document' : 'Upload Big Data Document'}
            </h2>
            <p className="text-sm text-slate-500 mt-1">
              {editMode.isEditing ? `Editing: ${title}` : 'Manage bulk datasets, predictive modeling exports, and analytical reports.'}
            </p>
          </div>
          {editMode.isEditing && (
            <button
              onClick={handleCancelEdit}
              className="text-sm font-bold text-rose-500 hover:bg-rose-100 px-4 py-2 bg-rose-50 rounded-lg transition-colors"
            >
              Cancel Edit
            </button>
          )}
        ...
        <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-12 gap-4 items-end">
          <div className="md:col-span-5">
            <label className="block text-sm font-semibold text-slate-700 mb-1.5">Dataset Title</label>
            <input
              type="text"
              value={title}
              onChange={(e) => setTitle(e.target.value)}
              required
              placeholder="e.g., Q1 Predictive Quality Data"
          ...
          <div className="md:col-span-4">
            ...
            <input
              ...
              className="block w-full text-sm text-slate-500 file:mr-4 file:py-3 file:px-4 file:rounded-xl file:border-0 file:font-semibold file:bg-fuchsia-50 file:text-fuchsia-700 hover:file:bg-fuchsia-100 transition-all border border-slate-200 rounded-xl bg-slate-50 cursor-pointer"
            />
          </div>
          <div className="md:col-span-3">
            <button
              type="submit"
              disabled={isLoading}
              className={`w-full text-white font-bold py-3 px-4 rounded-xl shadow-sm transition-all flex items-center justify-center gap-2 ${
                editMode.isEditing ? 'bg-fuchsia-600 hover:bg-fuchsia-700' : 'bg-slate-800 hover:bg-slate-900'
            ...
            >
              ...
              {isLoading ? 'Processing...' : editMode.isEditing ? 'Save Changes' : 'Upload Data'}
            </button>
          </div>
        </form>
      </div>
        ...
        <div className="px-6 py-5 border-b border-slate-100 bg-slate-50 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div className="flex items-center gap-2 text-sm font-bold text-slate-700">
            ...
            <select
              ...
              className="border border-slate-200 rounded-lg p-1.5 focus:ring-2 focus:ring-fuchsia-500 outline-none"
            >
          ...
          <div className="relative w-full sm:w-64">
            ...
            <input
              ...
              className="w-full pl-9 pr-4 py-2 border border-slate-200 rounded-xl text-sm focus:ring-2 focus:ring-fuchsia-500 outline-none transition-all bg-white"
            />
          </div>
        </div>
        ...
        <div className="overflow-x-auto">
          {isFetching ? (
            <div className="flex flex-col items-center justify-center py-16 gap-3">
              <svg className="animate-spin w-8 h-8 text-fuchsia-500" fill="none" viewBox="0 0 24 24">
                <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" />
                <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z" />
              </svg>
            ...
            <table className="w-full text-left text-sm text-slate-600">
              <thead className="bg-white border-b border-slate-200 text-xs uppercase tracking-wider text-slate-500 font-bold">
                <tr>
                  <th className="py-4 px-6 cursor-pointer select-none hover:text-fuchsia-600 w-1/2" onClick={() => toggleSort('title')}>
                    Dataset Title <SortArrow active={sortField === 'title'} direction={sortDirection} />
                  </th>
                  <th className="py-4 px-6 cursor-pointer select-none hover:text-fuchsia-600" onClick={() => toggleSort('uploadDate')}>
              ...
              <tbody className="divide-y divide-slate-100">
                {currentItems.length === 0 && !isFetching ? (
                  <tr>
                    <td colSpan="3" className="py-16 text-center">
                      <div className="flex flex-col items-center justify-center">
                        ...
                        <p className="text-slate-500 font-medium">No datasets found</p>
                        {searchTerm && (
                          <button onClick={() => setSearchTerm('')} className="mt-2 text-fuchsia-600 hover:text-fuchsia-700 text-sm font-semibold">
                            Clear Search
                          </button>
                        )}
                      </div>
                  ...
                  currentItems.map((doc) => (
                    <tr key={doc.id} className="hover:bg-fuchsia-50/30 transition-colors">
                      ...
                      <td className="py-4 px-6 flex justify-center gap-2">
                        <button onClick={() => handleViewPdf(doc.filename, doc.title)} className="p-2 text-slate-400 hover:text-fuchsia-600 hover:bg-fuchsia-50 rounded-lg transition-all" title="View PDF">
                          <EyeIcon />
                        </button>
                        <button onClick={() => handleEditClick(doc)} className="p-2 text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 rounded-lg transition-all" title="Edit">
                          <EditIcon />
                        </button>
        ...
        {!isFetching && filteredAndSortedData.length > 0 && (
          <div className="p-4 border-t border-slate-100 bg-slate-50 flex items-center justify-between text-sm text-slate-600">
            <div>
              ...
              <span className="font-bold text-slate-800">{Math.min(indexOfLastItem, filteredAndSortedData.length)}</span> of{' '}
              <span className="font-bold text-slate-800">{filteredAndSortedData.length}</span> entries
            </div>
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-6xl h-full flex flex-col overflow-hidden">
            <div className="p-4 border-b border-slate-100 bg-slate-50 flex justify-between items-center">
              <h3 className="font-bold text-slate-800 flex items-center gap-2">
                <DatabaseIcon className="w-5 h-5 text-fuchsia-600" /> {viewModal.title}
              </h3>
              <button
                onClick={handleCloseModal}
                className="bg-slate-200 hover:bg-rose-500 hover:text-white text-slate-600 w-8 h-8 rounded-lg flex items-center justify-center transition-colors"
              >
                &times;
              </button>
            </div>
            <div className="flex-1 bg-slate-200 flex items-center justify-center relative">
              {viewModal.isLoadingPdf ? (
                <div className="flex flex-col items-center gap-3">
                  ...
                  <p className="text-slate-600 font-semibold animate-pulse">Loading PDF...</p>
                </div>
              ) : (
                <object data={viewModal.fileUrl} type="application/pdf" className="w-full h-full">
                  <div className="flex flex-col items-center justify-center h-full space-y-4 p-8">
                    ...
                    <a
                      ...
                      download={`${viewModal.title}.pdf`}
                      className="px-6 py-2 bg-fuchsia-600 text-white rounded-lg hover:bg-fuchsia-700 shadow-sm transition-colors"
                    >
                      Download to View
                    </a>
                  </div>
